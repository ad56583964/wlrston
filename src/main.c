...
{
	int (*module_init)(struct wlrston_server *server,
			   int *argc, char *argv[]);

	module_init = load_module_entrypoint(name, "wlrston_module_init", MODULEDIR);
	if (!module_init)
		return -1;
...
load_shell(struct wlrston_server *server, const char *name, int *argc, char *argv[])
{
	...
	shell_init = load_module_entrypoint(name, "wlrston_shell_init", MODULEDIR);
	if (!shell_init)
		return -1;
